...
const CATALOG_H = path.join(ROOT, 'src', 'c', 'engine', 'catalog.h');
const SOURCE_JSON = path.join(ROOT, 'src', 'data', 'module-vibrant.json');
const OUT_C = path.join(ROOT, 'src', 'c', 'engine', 'vibrant_table.g.h');
const OUT_JS = path.join(ROOT, 'src', 'pkjs', 'clay', 'vibrant.g.js');
...
function main(): void {
  ...
  fs.writeFileSync(OUT_JS, buildJsMap(order, source));
  console.log('wrote ' + path.relative(ROOT, OUT_C) + ' and ' + path.relative(ROOT, OUT_JS));
}
