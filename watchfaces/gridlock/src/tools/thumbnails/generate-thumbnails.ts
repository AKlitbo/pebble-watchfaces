...
import moduleMeta from '../../pkjs/clay/module-meta.ts';
...
const SHOTS_DIR = path.join(ROOT, 'resources', 'thumbnails');
export const OUT_FILE = path.join(ROOT, 'src', 'pkjs', 'clay', 'module-thumbnails.g.js');
 ...
 */
export function buildFromShots(): Built {
  const bySlug = indexBySlug(moduleMeta);

  const files = fs.existsSync(SHOTS_DIR)
    ? fs.readdirSync(SHOTS_DIR).filter((name) => name.toLowerCase().endsWith('.png'))
    : [];
  ...
  shots.forEach((shot) => {
    const data = fs.readFileSync(path.join(SHOTS_DIR, shot.file)).toString('base64');
...
export function build(): void {
  ...
  fs.writeFileSync(OUT_FILE, built.source);

  const bytes = fs.statSync(OUT_FILE).size;
  console.log('wrote ' + OUT_FILE + ' (' + Math.round(bytes / 1024) + ' KB)');
