...
function firstManifest() {
  const manifestPath = findManifests()[0];
...
describe('buildComponentSource', () => {
  ...
  test('wraps the bundle and calls init with the component this', () => {
    const manifestPath = findManifests()[0];

    const result = buildComponentSource(manifestPath);
...
describe('generated components', () => {
  findManifests().forEach((manifestPath) => {
    /** A piece edit without gen:clay would ship a config page that ignores the change. */
    test(`${manifestPath.replace(/\\/g, '/').split('/').pop()} output is not stale`, () => {
