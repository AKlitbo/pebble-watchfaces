...
{
    // a stack buffer, not a static one. vibes_enqueue_custom_pattern queues the pattern to play
    // later, so it copies the durations rather than holding our pointer, and this can go once the
    // call returns
    uint32_t buf[GOAL_VIBE_MAX_SEGMENTS];
    int      count = 0;
    uint32_t value = 0;
    bool     have = false;
