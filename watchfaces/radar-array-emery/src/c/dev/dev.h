/**
 * @file dev.h
 * @brief Per-face dev-walk switches. Keep DEV_MODE 0 for any build you ship. Flip it (plus at
 * most one walk toggle) to boot the face into a fixed fixture and tap-walk themes or icons for
 * screenshots. The logic lives in lib `dev/dev_walk`. This only holds the switches.
 *
...
// ===== walk toggles (turn on at most ONE) =====
#define DEV_TAP_WALK_THEMES 0  // tap steps through every theme on fixed data + a fixed clock
#define DEV_TAP_WALK_ICONS  0  // tap steps through every weather condition (forces one theme)

// ===== icon walk: which theme to sit the icons on =====
#define DEV_ICON_WALK_THEME 0
...
#elif DEV_TAP_WALK_ICONS
  #define DEV_WALK_MODE DEV_WALK_ICONS
#else
...
{
#if DEV_MODE
    dev_walk_init(DEV_WALK_MODE, DEV_ICON_WALK_THEME, apply_theme);
#else
    (void)apply_theme;
#endif
}
