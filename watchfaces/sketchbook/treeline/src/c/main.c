/**
 * @file main.c
 * @brief treeline entry point: wires services to their stores and drives the slot engine
 * directly (no shell).
 *
 * The scene reacts to the weather, so it polls every 30 min, and it reacts to the sun, so the
 * minute tick also watches for the palette flipping at sunrise and sunset. The smoke leans on
 * the wind reading, which arrives with the weather, so it costs no timer of its own. It never
...
{
    // treeline owns its schema: v1 with its own key and no migration. the readout-style date
    // default ("SAT 19 JUN") lives in the schema's field table
    settings_init(treeline_settings_schema());
    ...
    treeline_setup(s_window);              // fonts + palette
    engine_init(s_window, treeline_build); // build the scene, chrome, and text slots
    ...
    dev_start(treeline_apply_theme);
}
