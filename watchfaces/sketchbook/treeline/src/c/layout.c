/**
 * @file layout.c
 * @brief Treeline face: the zone table bound to shared readouts, the drawn scene, and the
...
// --- Zone Table ---
// the clock on the clearing with the date beneath it and the stats row along the bottom. the colours
 ...
 *
 * The clock sits on the clearing floor and everything else sits over the sky, which is why the clock
...
static void apply_theme_colors(void)
{
    ...
    // the date-up layouts move it off the ground onto the strip, so it takes the strip's ink
...
{
    uint8_t layout = treeline_layout();
