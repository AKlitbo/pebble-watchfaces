/**
 * @file main.c
 * @brief shoreline entry point: wires services to their stores and drives the slot engine
 * directly (no shell).
 *
 * The scene reacts to the weather, so it polls every 30 min, and it reacts to the sun, so the
 * minute tick also watches for the palette flipping at sunrise and sunset. The tide, the waves
 * and the boat all ride the same minute tick, so the water costs no timer of its own. It never
 * displays coordinates, so on_coords is left unset and the JS sends none.
 */
#include <pebble.h>
 ...
 *
 * Two things can want the .beats ticker and they are independent: the clock itself, and a date
 * format ending in a reading. A beat is 86.4 seconds, so a date line left on the minute tick
 * sits on a stale number and reports every rollover up to a minute late. The store runs both
 * cadences at once, so the date line can have its ticker while the clock keeps its minute tick.
 */
static TimeConfig time_cfg(void)
{
    ...
    return (TimeConfig){
        ...
        .beats = clock_beats || readout_date_shows_beats(),
    };
}
...
{
    // shoreline owns its schema: v1 with its own key and no migration. the readout-style date
    // default ("SAT 19 JUN") lives in the schema's field table
    settings_init(shoreline_settings_schema());
    ...
    shoreline_setup(s_window);              // fonts + palette
    engine_init(s_window, shoreline_build); // build the scene, chrome, and text slots
    ...
    dev_start(shoreline_apply_theme);
}
