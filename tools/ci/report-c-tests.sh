...
if [ ! -f "$LOG" ]; then
  echo "no test log at $LOG: run 'make -C lib/c/spec | tee $LOG' first" >&2
  exit 1
fi
...
if ! grep -qE "$COUNTER" "$LOG"; then
  echo "no Unity results in $LOG: the suite did not run" >&2
  exit 1
fi
...
report=$(awk '
  $0 ~ /[0-9]+ Tests [0-9]+ Failures [0-9]+ Ignored/ {
    files++; total += $1; fails += $3; ignored += $5;
    if ($3 == 0) { filespass++; }
  }
  END {
