...
* **`watchfaces/<face>/`**: one watchface, self-contained.
  ...
  * **`src/tools/`**: generators for assets only that face uses, run by its own `npm run gen:*`.
...
* **`targets/<target>/`**: the build sandbox waf runs in, one per build target. Entirely generated and gitignored. Its `package.json`, `wscript`, and `emit/` are all produced by the build. A face usually declares one target named after itself, so this is usually `targets/<face>/`. A face that declares a `targets` map in its appinfo gets one sandbox per target instead, each carrying a `.source-face` marker naming the face it was built from.
...
* **Gridlock**
  ...
  * **Glyphs**: Heart, step, distance, thermometer, UV, fire, snooze, late, clock, globe, and volume icons from [UXWing](https://uxwing.com).
* **Sketchbook Faces**
