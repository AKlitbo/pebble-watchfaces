...
import { codeFor, UNKNOWN_CODE, FORECAST_NIGHT_BIT } from '../conditions.js';
...
describe('parseForecast', () => {
  ...
  test('starts the hourly strip at the first slot at or after now', () => {
    ...
    expect(result.hourly.cols[0]).toEqual({ code: codeFor('RAIN'), temp: 12 });
    expect(result.hourly.cols[1]).toEqual({ code: codeFor('PCLDY'), temp: 14 });
  });
  ...
  test('sets the night bit on an hourly column whose is_day is 0', () => {
    ...
    expect(result.hourly.cols[0].code).toBe(codeFor('CLEAR') | FORECAST_NIGHT_BIT);
  });
  ...
  test('leaves the night bit off an hourly column whose is_day is 1', () => {
    ...
    expect(result.hourly.cols[0].code).toBe(codeFor('CLEAR'));
  });

  /** An unknown sky has no night glyph, so a night hour must leave UNKNOWN_CODE untouched. */
  test('never sets the night bit on an unknown-condition column', () => {
    ...
    expect(result.hourly.cols[0].code).toBe(UNKNOWN_CODE);
  });
  ...
  test('reads the daily strip with its base weekday and highs/lows', () => {
    ...
    expect(result.daily.cols[0]).toEqual({ code: codeFor('CLEAR'), tempMax: 25, tempMin: 15 });
    expect(result.daily.cols[1]).toEqual({ code: codeFor('RAIN'), tempMax: 22, tempMin: 13 });
  });
...
describe('openmeteo provider', () => {
  ...
  describe('successful reads', () => {
    ...
    test('derives PCLDY from a partly-cloudy weather_code', () => {
