 ...
 * @return {Function}
 */
function replying(response, calls) {
  return (url, callback) => {
    calls.push(url);
...
describe('finnhub provider', () => {
  ...
  describe('isoDateFromUnix', () => {
    /** A wrong date maths would label a quote with the wrong trading day. */
    test('formats a unix time as a plain date', () => {
      const result = finnhub.isoDateFromUnix(86400);
    ...
    test('returns blank for a zero timestamp', () => {
      const result = finnhub.isoDateFromUnix(0);
  ...
  describe.skipIf(process.env.RUN_LIVE_STOCK !== '1' || !process.env.FINNHUB_KEY)('live', () => {
    ...
    const live = (opts) => new Promise((resolve) => finnhub.fetch(opts, fetchRequest, resolve));
